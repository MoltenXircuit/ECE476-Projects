...
#─────[ Update Array Data ]────────────────────────────────────
def UpdateKeyboard(tune = 0):
    for i in (range(0,N)):
        if ((key_time[i] == 0) or (key_time[i] == 255)):   #leave if in invalid space
            break 
        if (key_time[i] < key_time_min[i]):   #update min
        ...
        key_scaled_state[i] = int( (key_time[i]-key_time_min[i]) * 128 / key_scale_array[i])
        if key_scale_array[i] < 20:
            break
        ...
        elif( (previous_state[i] != 0) and (key_scaled_state[i] < (50)) ):     #add key_off to queue if not already off
            delta_key_stack.append([i,0])
            previous_state[i] = 0
            
        print("scaled_key", key_scaled_state)
        print("stack", delta_key_stack)
